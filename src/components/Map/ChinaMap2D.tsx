    ...
    <div className="relative w-full h-full">
      ...
      >
        ...
        {PROVINCE_GEOMETRIES.map((province) => {
          ...
          // 简化的省份形状（多边形）
          const [x1, y1] = project(province.bounds[0], province.bounds[1]);
          const [x2, y2] = project(province.bounds[2], province.bounds[1]);
          const [x3, y3] = project(province.bounds[2], province.bounds[3]);
          const [x4, y4] = project(province.bounds[0], province.bounds[3]);
          
          const size = Math.max(Math.abs(x2 - x1), Math.abs(y4 - y1)) * 0.6;
          const [cx, cy] = project(province.center[0], province.center[1]);
          
          // 实际项目中使用真实路径，这里用圆角矩形模拟省份
          return (
            <g key={province.id}>
              <rect
                x={cx - size/2}
              ...
              {isVisited && (
                <g>
                  ...
                  {/* 旗帜 */}
                  <polygon
                    points={`${cx - size/3},${cy - size/3} ${cx + size/8},${cy - size/4} ${cx - size/3},${cy - size/6}`}
                    fill="#ef4444"
                    style={{
